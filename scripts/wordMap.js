(function(){
  ...
  var engine = (function(){
    ...
    //-- init
    function init(){
      dataURL = (window.location.href.substr(0,4).toLowerCase() == "file")? "data/test.json" : "http://www.whoin.net/tweetTweetCalls/getTweets.php";
      loadData(dataURL);
    }
    ...
    function visualize(dataObj) {
      ...
      var mousemove = function(d) {
        ...
        var yPosition = (d3.event.pageY < (sHeight + 25))? d3.event.pageY + 12 : d3.event.pageY - 50;
        ...
        wordMap.selectAll(".node").style("cursor", "pointer");
      };

      var mouseout = function() {
        d3.select("#tip").classed("hidden", true);

        wordMap.selectAll(".node").style("cursor", "default");
      };
      ...
      var node = wordMap.datum(dataObj).selectAll(".node")
                ...
                .on('click', function(d, i) {
                  if (d.name.substr(0,1) == "#") {
                    window.open("https://twitter.com/hashtag/" + d.name.substr(1) + "?src=hash");   //-- https://twitter.com/hashtag/machineeatable?src=hash
                  } else if (d.name.substr(0,1) == "@") {
                    window.open("https://twitter.com/" + d.name.substr(1));                         //-- https://twitter.com/IBMWatson
                  }
                  return;
                }) 
